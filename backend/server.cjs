...
dotenv.config(); // Carga las variables de entorno desde .env en desarrollo local

const app = express();
// Cloud Run inyecta la variable de entorno 'PORT'.
// Usamos process.env.PORT, y si no está definido (ej. para desarrollo local),
// usamos 8080 como fallback (coincidiendo con el puerto que expone tu Dockerfile).
const port = process.env.PORT || 8080; // CAMBIO: Usar 8080 como fallback consistente

// --- CONFIGURACIÓN DE AUTENTICACIÓN ---
// Tu ID de Cliente OAuth 2.0 de la Consola de Google Cloud
const GOOGLE_CLIENT_ID = process.env.GOOGLE_CLIENT_ID; 

// ¡IMPORTANTE! Este valor en Cloud Run DEBE ser el mismo que el largo que tienes aquí.
// Idealmente, en producción, solo deberías tener process.env.JWT_SECRET; sin fallback hardcodeado.
...
// Es crucial para permitir que tu frontend (en un dominio diferente) acceda a este backend.
// Añade todas las URLs de origen permitidas.
app.use(cors({
  origin: [
    'http://localhost:5173', // Para desarrollo local del frontend
    'https://gcp-vm-dashboard-frontend-service-780691668337.europe-southwest1.run.app' // ¡Tu URL de frontend en Cloud Run!
  ]
}));
...
app.get('/', (req, res) => {
  res.status(200).send('OK'); // Responde con 200 OK para el health check de Cloud Run
});
// --- FIN NUEVA RUTA ---
...
// Obtener el ID del proyecto de la variable de entorno
...
    instancesClient = new computePackage.v1.InstancesClient({ projectId: GCP_PROJECT_ID }); // <--- CORRECCIÓN CLAVE
...
    zonesClient = new computePackage.v1.ZonesClient({ projectId: GCP_PROJECT_ID }); // <--- CORRECCIÓN CLAVE
...
    globalOperationsClient = new computePackage.v1.GlobalOperationsClient({ projectId: GCP_PROJECT_ID }); // <--- CORRECCIÓN CLAVE
...
// --- FIN RUTA DE AUTENTICACIÓN ---


// --- Rutas de la API de VMs (PROTEGIDAS) ---
