...
const App: React.FC = () => {
  ...
  const [appToken, setAppToken] = useState<string | null>(localStorage.getItem('appToken')); // Cargar token al inicio
  const [userEmail, setUserEmail] = useState<string | null>(localStorage.getItem('userEmail')); // Cargar email al inicio
  const [isAuthenticating, setIsAuthenticating] = useState(false); // Para mostrar spinner en login
  // --- FIN ESTADO DE AUTENTICACIÓN ---

  const GCP_PROJECT_ID_REAL = 'puestos-de-trabajo-potentes'; 
  // --- ¡TU CLIENT ID DE OAUTH 2.0! COPIALO DE LA CONSOLA DE GOOGLE CLOUD ---
  ...
  const loadVMs = useCallback(async () => {
    ...
    try {
      ...
      // Si el error es de autenticación, limpiar el token y forzar el login
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
              ...
              <div className="mt-4 md:mt-0 md:col-start-2 lg:col-start-auto">
                 <button
                    onClick={loadVMs}
                    disabled={isLoading}
                    className="w-full flex items-center justify-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-gcp-blue hover:bg-sky-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-sky-500 disabled:opacity-50"
                  >
